...
public class PhoneBook {
    ...
    public int searchRecords(int searchMenuChoice, String searchStr) {
        int recordsFound=0;
        String record;
        List<Integer> foundListIndex = new  ArrayList<>();

        int index=0;
        if (searchMenuChoice==1)
        {
        for(Person person : persons) {
            if(person.getFio().contains(searchStr)) {
                foundListIndex.add(index);
            }
            index++;
            }
        }
        ...
        {
            for(Person person : persons) {
                LinkedHashSet<String> phoneList = person.getPhones();
                for(String phone: phoneList){
                    record = phone;
                    if(record != null && !record.isEmpty())
                    if(record.contains(searchStr)){
                        foundListIndex.add(index);
                        break;
                    }
                }
        ...
        recordsFound = foundListIndex.size();

        for(int ind : foundListIndex) {
            System.out.println("Запись " + ind);
            System.out.print(persons.get(ind));
        }
        System.out.println("--- Записей найдено: " + recordsFound+ " ---");
